...
async function add15minCache(line_id,cb){
  ...
  if((new Date() - last_update <= 15000) && shift_res){
    ...
    var start;
    if (new Date(batch_data.start_time) > new Date(d)) {
      start = moment(batch_data.start_time).local().subtract(1, "days").format("YYYY-MM-DD");
    } else {
      start = moment(batch_data.start_time).format("YYYY-MM-DD");
    }
    ...
    data.forEach((element, i) => {
      ...
      var current_duration = moment().diff(moment(machine_condition.last_update), "seconds");
     ...
     if(element.fault_arr){
      ...
      var cal_data = calculation(element.goodCount, element.reject_count, element.total_batch_duration, element.pdt, element.changeover, element.updt, element.major_fault,element.major_fault_count,element.minor_fault,element.major_manual_stop,element.minor_manual_stop,element.blocked,element.waiting, batch_data.product_name.rated_speed,element.startup_reject,element.ready)
      send_data['fault_1'] = "-";
      send_data['fault_2'] = "-";
      send_data['fault_3'] = "-";
      send_data['fault_4'] = "-";
      send_data['fault_5'] = "-";
      send_data['fault'] = `${element.major_fault_count} / ${convertHHMM(element.major_fault)}`;
      ...
      send_data['fgex_details'] = batch_data.product_name.fgex +' / '+ batch_data.product_name.rated_speed ;
      ...
      send_data['waiting'] = `${element.waiting_count} / ${convertHHMM(element.waiting)}`;
      send_data['count'] = `${element.goodCount} / ${element.reject_count}`
      send_data['blocked'] = `${element.blocked_count} / ${convertHHMM(element.blocked)}`;
      send_data['changeover_time'] = `${convertHHMM(element.changeover)}`;
      send_data['ready'] = `${convertHHMM(element.ready)}`;
      send_data['batch_manual_stop'] = `${element.major_manual_stop_count } / ${convertHHMM(element.major_manual_stop  )}`;
      send_data['batch_minor_manual_stop'] = `${element.minor_manual_stop_count} / ${convertHHMM( element.minor_manual_stop)}`;
      send_data['minor_fault'] = `${element.minor_fault_count} / ${convertHHMM(element.minor_fault)}`;
      send_data['batch_start_time'] = moment(batch_data.start_time).format("DD-MM-YYYY - HH:mm:ss");
      send_data['pdt'] = `${element.pdt_count} / ${convertHHMM(element.pdt)}`;
      send_data['shift'] = shift;
      send_data['updt'] = `${element.updt_count} / ${convertHHMM(element.updt)}`;
      ...
      send_data['idel_time'] = (element.minor_manual_stop_count + element.blocked_count + element.waiting_count + element.minor_fault_count + element.ready_count) +' / '+ cal_data.idel_time  ;
      ...
      send_data['speed_blisters'] = machine_temp.bpm * batch_data.product_name.blister_per_format;
      ...
      sort_arr.forEach(fault => {
        if (fault.fault_name) {
          send_data[`fault_${count}`] = `${fault_obj[element._id][fault.fault_name]}  ${fault.count} / ${convertHHMM(Math.round(fault.duration))} `;
          count++
        }
      });
...
function calculation(goodCount, reject_count, total_batch_time, pdt, changeover, updt, major_fault, major_fault_count,minor_fault,major_manual_stop,minor_manual_stop,blocked,waiting, rated_speed,changeover_reject,ready) {
  ...
  var performance_time = convertMinutes(production_time) - (total_count / rated_speed);
  var reject_time =  Math.floor((reject_count / rated_speed) * 60) ;
  var changeover_wastage_time = Math.floor((changeover_reject / rated_speed) * 60) ;
  ...
  data.performance = checkValidation(total_count / (rated_speed * convertMinutes(production_time)));
