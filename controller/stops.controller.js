...
async function add15minCache(startDate,endDate){
    if(startDate === api_strat_date && api_end_date == endDate ){
        return stop_res
    }else{
        //console.log(new Date() - last_update,stop )
        if((new Date() - last_update >= 15000)){
            return stop_res
        }else{
            ...
            for (var i = 0; i < stop.length; i++) {
                ...
                if (stop[i].stop_name.match(/^fault/g)) {
                    ...
                    var data = {
                        stop_name: name,
                        _id: stop[i]._id,
                        full_fault_des: faultDescritions[stop[i].machine_name][stop[i].stop_name],
                        short_fault_des: faultDescritions[stop[i].machine_name][stop[i].stop_name],
...
router.get('/new', async (req, res) => {
    ...
    if (new Date(startDate) > new Date()) {
        ...
        var send_data = await add15minCache(startDate,endDate);
        res.send(send_data);
    }
});
        ...
        stop.forEach((element) => {
            element.details.forEach((fault) => {
                ...
                if (machine_filter.length == 0) {
                    ...
                    if (fault.stop_name.match(/^fault/g)) {
                        var fault_name = stop_name.details.find(x => x.stop_name === 'fault');
                        if (!fault_name) {
                            ...
                            fault_name.count_wise.sort((a, b) => (a.count > b.count) ? -1 : 1);
                            fault_name.duration_wise.sort((a, b) => (a.duration > b.duration) ? -1 : 1);
                        }

                    } else if (fault.stop_name.match(/^waiting_/g)) {
                        var fault_name = stop_name.details.find(x => x.stop_name === 'waiting');
                        if (!fault_name) {
                            stop_name.details.push({
                                stop_name: 'waiting',
                                total: fault.total,
...
router.get('/shift/:id', async (req, res) => {
    ...
    var sku_data = await skuAdd.find({ sku_end_time: null, machine_name: machine });
    sku_data.forEach(async (sku) => {
        ...
        var total_good_count = sku.current_count + temp.current_value - sku_add.sku_start_count;
        var total_reject_count = tempReject.current_value - sku_reject.sku_start_count;
        sku.current_count = total_good_count + total_reject_count;
        stop_name['sku_wise'] = sku;
    });
    var project_res = await Project.find({ shiftName: shift, date: d, machine_name: machine });
    project_res.forEach(async (project, i) => {
        ...
        var stop_temp = await Temp.findOne({ tag_name: /^fault/, machine_name: machine, current_state: true });
        var pdt_temp = await Temp.findOne({ tag_name: 'pdt', machine_name: machine });
        var condition = await getCondition(machine);
        var total_good_count = project.goodCount + temp.current_value - temp.shift_Start_count;
        var total_reject_count = project.rejected_quantity + tempReject.current_value - tempReject.shift_Start_count;
        ...
        var packing_machine = ['case_packer', 'case_erector', 'weigher', 'case_sealer'];
        var rated_speed;
        ...
        if (stop_temp) {
            ...
            var total_stop_temp = Math.abs(minutes - (stop_timestamp.getHours() * 60 + stop_timestamp.getMinutes()));
            fault_time += total_stop_temp
        }
        if (pdt_temp.current_state && pdt_temp) {
            var pdt_timestamp = new Date(pdt_temp.timeStamp);
            var total_pdt_temp = Math.abs(minutes - pdt_timestamp.getHours() * 60 + pdt_timestamp.getMinutes());
        ...
        var performance = total_good_count / (rated_speed * (working_time - fault_time));
        ...
        if (machine == 'siapi') {
            var waiting_temp = await Temp.findOne({ tag_name: /^waiting_/, machine_name: machine, current_state: true });
            var blocked_temp = await Temp.findOne({ tag_name: 'blocked', machine_name: machine });
            var waiting_time = project.waiting;
            var blocked_time = project.blocked;
            if (waiting_temp) {
                var waiting_timestamp = new Date(waiting_temp.timeStamp);
                var total_waiting_temp = Math.abs(minutes - (waiting_timestamp.getHours() + waiting_timestamp.getMinutes()));
                waiting_time += total_waiting_temp;
            }
            if (blocked_temp.current_state) {
                var blocked_timestamp = new Date(blocked_temp.timeStamp);
                var total_blocked_temp = Math.abs(minutes - blocked_timestamp.getHours() + blocked_timestamp.getMinutes());
                blocked_time += total_blocked_temp;
            }
            var actual_run_time = total_time - fault_time - pdt_time - waiting_time - blocked_time;
            var speed_loss = actual_run_time - (total_count / rated_speed);
            var time_loss = fault_time + waiting_time + blocked_time + speed_loss + (total_reject_count / rated_speed);
            var bottle_loss = time_loss * rated_speed;
            var send_bottle_loss;
            if (bottle_loss > all_max_bottle_loss.bottle_loss || all_max_bottle_loss.bottle_loss == 0) {
...
router.get('/report', async(req,res)=>{
    ...
    shiftWiseCalculation(date,shift,(day_wise_response)=>{
        ...
        send_data['Bottle_loss']['Total'] = Math.round(day_wise_response.critical_stop.bottle_loss);
        ...
        day_wise_response.critical_stop.stop.forEach(element => {
            ...
            if (element.stop_total > major_minor_duration) {
                send_data['Bottle_loss']['Availability']['major_fault'][faultDescritions[day_wise_response.critical_stop.machine_name][element.stop_name].full] = bottle_loss;
                send_data['Bottle_loss']['Availability']['fault']['total'] += bottle_loss;
                send_data['Bottle_loss']['Availability']['major_fault']['total'] += bottle_loss;
                //send_data['Bottle_loss']['Total'] += bottle_loss;
                send_data['Bottle_loss']['Availability']['total'] += bottle_loss;
            } else {
                send_data['Bottle_loss']['Availability']['minor_fault'][faultDescritions[day_wise_response.critical_stop.machine_name][element.stop_name].full] = bottle_loss;
                send_data['Bottle_loss']['Availability']['minor_fault']['total'] += bottle_loss;
                send_data['Bottle_loss']['Availability']['fault']['total'] += bottle_loss;
                //send_data['Bottle_loss']['Total'] += bottle_loss;
                send_data['Bottle_loss']['Availability']['total'] += bottle_loss;
            }
        ...
        day_wise_response.critical_stop.waiting.forEach(element => {
            var bottle_loss =  Math.round(element.stop_total * rated_speed);
            send_data['Bottle_loss']['Performance']['waiting']['total'] += bottle_loss;
            //send_data['Bottle_loss']['Total'] += bottle_loss;
            send_data['Bottle_loss']['Performance']['total'] += bottle_loss;
            send_data['Bottle_loss']['Performance']['waiting'][faultDescritions[day_wise_response.critical_stop.machine_name][element.stop_name].full] = bottle_loss;
        });
        //blocked loss
        day_wise_response.critical_stop.blocked.forEach(element => {
            var bottle_loss =  Math.round(element.stop_total * rated_speed);
            send_data['Bottle_loss']['Performance']['blocked']['total'] += bottle_loss;
            //send_data['Bottle_loss']['Total'] += bottle_loss;
            send_data['Bottle_loss']['Performance']['total'] += bottle_loss;
            send_data['Bottle_loss']['Performance']['blocked'][faultDescritions[day_wise_response.critical_stop.machine_name][element.stop_name].full] = bottle_loss;
        });
        day_wise_response.parameter_wise.forEach((element, i) => {
            if (element._id.match('^fault')) {
                if (element.details.length > 1) {
                    element.details.forEach((fault) => {
                        if (counter == 1) {
                            ...
                            send_data['fault' + counter][fault.machine_name] = faultDescritions[fault.machine_name][element._id].full + " " + fault.count + "/" + fault.total.toFixed(2);
                            counter++
                        } else {
                                ...
                                if (send_data.hasOwnProperty(key)) {
                                    if (key.match('^fault')) {
                                        if (send_data[key][fault.machine_name] == "-") {
                                            send_data[key][fault.machine_name] = faultDescritions[fault.machine_name][element._id].full + " " + fault.count + "/" + fault.total.toFixed(2);
                                            return
                                        }
                    ...
                    if (counter == 1) {
                        send_data['fault' + counter] = {...machine_obj};
                        send_data['fault' + counter][element.details[0].machine_name] = faultDescritions[element.details[0].machine_name][element._id].full + " " + element.details[0].count + "/" + element.details[0].total.toFixed(2);
                        counter++
                    } else {
                        for (const key in send_data) {
                            if (send_data.hasOwnProperty(key)) {
                                if (key.match('^fault')) {
                                    if (send_data[key][element.details[0].machine_name] == "-") {
                                        send_data[key][element.details[0].machine_name] = faultDescritions[element.details[0].machine_name][element._id].full + " " + element.details[0].count + "/" + element.details[0].total.toFixed(2);
                                        return
                                    }
                ...
                element.details.forEach((machine) => {
                        send_data[element._id][machine.machine_name] = machine.count + "/" + Math.round(machine.total);
                })
            }
        });
        day_wise_response.project.forEach((data) => {
            ...
            if(data.machine !='vision'){
                send_data['Bottle_loss']['Quality']['reject'][data.machine] = data.manual_rejected_quantity;
                send_data['Bottle_loss']['Quality']['reject']['total'] += data.manual_rejected_quantity;
                send_data['Bottle_loss']['Quality']['total'] += data.manual_rejected_quantity;
            }
           ...
           // send_data['Bottle_loss']['Performance']['total'] += Math.round(data.critical_machine_off_time * rated_speed);
            ...
            send_data['Bottle_loss']['Performance']['blocked'][data.machine] = Math.round(data.critical_machine_off_time * rated_speed);
            ...
            send_data['manual_rejected_quantity'][data.machine] = data.manual_rejected_quantity;
            ...
            if (data.machine == day_wise_response.critical_stop.machine_name) {
                ...
                send_data['Bottle_loss']['Performance']['speed_loss']['total'] += data.speed_loss;
...
router.get('/dayreport', (req, res) => {
    ...
    day_wise_response.project.forEach((data) => {
        ...
        send_data['no_of_stop'][data.machine] = data.no_of_stop +"/"+data.stop;
        send_data['good_count'][data.machine] = data.goodCount;
        send_data['total_count'][[data.machine]] = (data.goodCount + data.manual_rejected_quantity);
        send_data['rejected_quantity'][data.machine] = data.rejected_quantity;
        send_data['manual_rejected_quantity'][data.machine] = data.manual_rejected_quantity;
        send_data['rework'][data.machine] = data.rework;
        send_data['manual_rework'][data.machine] = data.manual_rework;
...
var video_url = function (data) {
    ...
    var secound = check_number(Number(query.split("T")[1].split(":")[2].split(".")[0]));
    return video_server + year + month + date + hour + minute + secound
}

var cal_rated_speed = function (machine,sku) {
		var rated_speed;
		var packing_machine = ['tmgcp','weigher_case_sealer','weigher','case_sealer','vertilator'];
		if(packing_machine.indexOf(machine) == -1){
